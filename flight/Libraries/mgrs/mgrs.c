 ...
 *                                    (-180 to 360 degrees)
 *          MGRS_STR_ERROR         : An MGRS string error: string too int,
...
{ /* BEGIN Get_Grid_Values */
  ...
  int aa_pattern;    /* Pattern based on ellipsoid code */
...
{ /* BEGIN UTM_To_MGRS */
  ...
  int temp_error_code = MGRS_NO_ERROR;
  int error_code = MGRS_NO_ERROR;
  ...
  if (!error_code)
  {
    ...
    letters[1] = ltr2_low_value + ((int)(grid_easting / ONEHT) -1); 
