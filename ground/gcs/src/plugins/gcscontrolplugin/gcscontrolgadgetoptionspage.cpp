...
GCSControlGadgetOptionsPage::GCSControlGadgetOptionsPage(GCSControlGadgetConfiguration *config,
                                                         QObject *parent)
    : IOptionsPage(parent)
    , m_config(config)
{
...
QWidget *GCSControlGadgetOptionsPage::createPage(QWidget *parent)
{
    ...
    chList.clear();
    chList << options_page->channel0 << options_page->channel1 << options_page->channel2
           << options_page->channel3 << options_page->channel4 << options_page->channel5
    ...
    buttonActionOptions << "Does nothing"
                        << "Increases"
                        << "Decreases"
                        << "Toggles";
    ...
    for (i = 0; i < 8; i++) {
        ...
        connect(buttonFunctionList.at(i), SIGNAL(currentIndexChanged(int)), this,
    ...
    connect(buttonActionList.at(0), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_0()));
    connect(buttonActionList.at(1), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_1()));
    connect(buttonActionList.at(2), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_2()));
    connect(buttonActionList.at(3), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_3()));
    connect(buttonActionList.at(4), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_4()));
    connect(buttonActionList.at(5), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_5()));
    connect(buttonActionList.at(6), SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateButtonAction_6()));
...
void GCSControlGadgetOptionsPage::updateButtonAction(int controlID)
{
...

    {
        if (buttonActionList.at(controlID)->currentText().compare("Does nothing") == 0) {
        ...
        } else if (buttonActionList.at(controlID)->currentText().compare("Toggles") == 0) {
            disconnect(buttonFunctionList.at(controlID), SIGNAL(currentIndexChanged(int)), this,
                       SLOT(updateButtonFunction()));
            buttonOptions << "-"
                          << "Armed"
                          << "GCS Control"
            ...
            buttonLabelList.at(controlID)->setVisible(0);
            buttonValueList.at(controlID)->setVisible(0);
            connect(buttonFunctionList.at(controlID), SIGNAL(currentIndexChanged(int)), this,
                    SLOT(updateButtonFunction()));
        } else {
            disconnect(buttonFunctionList.at(controlID), SIGNAL(currentIndexChanged(int)), this,
                       SLOT(updateButtonFunction()));
