...
OPMapGadgetWidget::OPMapGadgetWidget(QWidget *parent) : QWidget(parent)
{
	...
	if (pm)
	{
        uavo_mgr = pm->getObject<UAVObjectManager>();
        uavo_util_mgr = pm->getObject<UAVObjectUtilManager>();
	}
    ...
    if (pm) {
        ...
        if (uavo_mgr) {
            UAVDataObject *obj = uavo_mgr->getObject<UAVDataObject>(QString("HomeLocation"));
            if (obj)
...
double OPMapGadgetWidget::getUAV_Yaw()
{
    ...
    UAVObject *obj = uavo_mgr->getRequiredObject(QString("AttitudeActual"));
    if (!obj)
        return 0;
...
bool OPMapGadgetWidget::setHomeLocationObject()
{
    ...
    return (uavo_util_mgr->setHomeLocation(LLA, false) >= 0);
}
