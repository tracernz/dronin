...
void ImportSummaryDialog::doTheApplySaving()
{
    ...
    for(int i=0; i < ui->importSummaryList->rowCount(); i++) {
        ...
        QCheckBox *box = dynamic_cast<QCheckBox*>(ui->importSummaryList->cellWidget(i,0));
        if (box->isChecked()) {
            UAVObject* importedObj = importedObjects->getRequiredObject(uavObjectName);
            UAVObject* boardObj = boardObjManager->getRequiredObject(uavObjectName);
            if (!importedObj || !boardObj) {
                qWarning() << "Failed to import " << uavObjectName;
                continue;
            }
